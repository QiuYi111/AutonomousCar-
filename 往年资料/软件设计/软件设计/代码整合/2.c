...
extern int find_way_order;
uint8_t Rx_BtBuffer;
int open_mv_output = 0;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	if(huart==&huart2){
		if(Rx_BtBuffer=='c')
			capture();
		else if(Rx_BtBuffer=='l')
			laydown();
		HAL_UART_Receive_IT(huart,&Rx_BtBuffer,1);
	}
	if(huart == &huart1){
		...
		if(output){
			...
			//printf("target=%d\n",target);
			//open_mv_output = target;
			//printf("open_mv_output = %d\n", open_mv_output);
			//if(open_mv_output == 1){
			//	fetch_flag = 1;
			//}
			/*
			if(openmv_flag == 1){
				find_way_order = target;
				openmv_flag = 0;
				printf("find_way_order = %d\n", find_way_order);
			}
			*/
			find_way_order = target;
			//openmv_flag = 0;
		...
		//if(fetch_flag == 0){
		//	HAL_UART_Receive_IT(huart,Rx_OpMV,size);
		//}
